...
def process_single_smiles(smiles_data):
    ...
    try:
        ...
        temp_file_timeout = tempfile.NamedTemporaryFile(delete=False, mode='w', prefix=f'batch_{idx}_timeout_')
        temp_file_timeout.write(smiles + '\n')
        temp_file_timeout.close()
        return {
            'temp_files': [temp_file_timeout.name],
            'time_taken': time_limit,
            'successful': False
        }
    except Exception:
        # Handle any other exceptions by writing to failures file
        temp_file_failure = tempfile.NamedTemporaryFile(delete=False, mode='w', prefix=f'batch_{idx}_failures_')
        temp_file_failure.write(smiles + '\n')
        temp_file_failure.close()
        return {
            'temp_files': [temp_file_failure.name],
            'time_taken': None,
            'successful': False
        }
    finally:
